...
public class EmployeeController {
    ...
    @GetMapping @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_HR_MANAGER')") public List<EmployeeResponse> all() { return employees.findAll().stream().map(EmployeeResponse::from).toList(); }
    @GetMapping("/{id}") @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_HR_MANAGER')") public EmployeeResponse one(@PathVariable UUID id) { return EmployeeResponse.from(employee(id)); }
    @PostMapping @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_HR_MANAGER')") public EmployeeResponse create(@Valid @RequestBody EmployeeRequest r) { if(employees.existsByEmployeeCode(r.employeeCode()) || employees.existsByEmail(r.email())) throw new IllegalArgumentException("Employee code or email already exists"); return EmployeeResponse.from(employees.save(new Employee(r.employeeCode(),r.firstName(),r.lastName(),r.email(),r.phone(),r.jobTitle(),r.hireDate(),department(r.departmentId())))); }
    @PutMapping("/{id}") @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_HR_MANAGER')") public EmployeeResponse update(@PathVariable UUID id, @Valid @RequestBody EmployeeRequest r) { Employee e=employee(id); e.update(r.firstName(),r.lastName(),r.email(),r.phone(),r.jobTitle(),r.hireDate(),department(r.departmentId()),r.status()); return EmployeeResponse.from(employees.save(e)); }
